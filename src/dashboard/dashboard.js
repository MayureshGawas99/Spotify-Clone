...
import { ENDPOINT, logout, SECTIONTYPE, setItemInLocalStorage, LOADED_TRACKS, getItemFromLocalStorage } from "../common";
...
const loadPlaylist =async(endpoint, elementId) => {
    ...
    for (let {name,description,images,id} of items ){
        ...
        playlistItem.className = "relative bg-black-secondary rounded p-4 hover:cursor-pointer hover:bg-light-black";
        playlistItem.id = id;
        ...
        <button class=" opacity-0 absolute right-6 -translate-y-12 text-green  "><span style="font-size: 36px" class="material-symbols-outlined bg-black rounded-full ">play_circle</span></button>
         <h2 class="text-base font-semibold mb-4 truncate">${name}</h2>
         <h3 class="text-sm text-secondary line-clamp-2">${description}</h3>`;
        playlistItemsSection.appendChild(playlistItem);
        playlistItem.addEventListener("mouseover",(id)=>{
            const popButton = playlistItem.querySelector(`button`);
            popButton.classList.remove("opacity-0");
        })
        playlistItem.addEventListener("mouseleave",(id)=>{
            const popButton = playlistItem.querySelector(`button`);
            popButton.classList.add("opacity-0");
        })
    }
}
...
const updateIconsForPlayMode = (id) =>{
    ...
    if (playButtonFromTracks){
        playButtonFromTracks.innerHTML = `pause`;
    }
}
...
const updateIconsForPauseMode = (id)=>{
    ...
    const playButtonFromTracks = document.querySelector(`#play-track-${id}`);
    if (playButtonFromTracks){
        playButtonFromTracks.innerHTML = `play_arrow`;
    }
}
...
const playNextTrack = () => {
    ...
    if (currentTrackIndex > -1 && currentTrackIndex< tracks.length -1){
        let {id} = tracks[currentTrackIndex];
...
const fillContentForPlaylist = async (playlistId) => {
    ...
    coverElement.innerHTML = `  <img class="object-contain w-50 h-50" src="${images[0].url}" alt="" srcset="">
                                <section>
    ...
    <header class="mx-8 border-secondary border-b-[0.5px] z-10" id="playlist-header">
        <nav class="py-2">
